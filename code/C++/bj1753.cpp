//1753 최단경로 - C++17
//메모리 : 15284KB / 시간 : 116ms
//2024년 2월 1일 23:03:58
...
int main() {
  ...
  cin.tie(0);
  cout.tie(0);
  long check[20001]; // 각 도시별 방문 확인 및 최저 가중치 확인 용도 배열
  fill(check, check+20001, -1e9); // check 배열을 전부 -1e9(어떤 경우에도 넘을 수 없는 값)로 저장
  vector<li> vt[20001]; // 벡터 배열, 인덱스 = 도시 번호 / 삽입된 정보 = {버스 , 가중치}
  priority_queue<li> pq; // li(long, int)자료형을 가진 우선순위 큐 선언
  int n, m, a, b, c, i, start, end;
  cin >> n >> m; // 도시 개수, 버스 개수 입력
  cin >> start; // 진짜 원하는 경로 입력
  for(i=0;i<m;i++) {
  ...
  while(!pq.empty()) { // 우선순위 큐가 빌때까지 반복
    ...
    // if(tnode == end) { // end노드에 도달했다면 출력하고 끝내기!! 
        ...
        // break;
    // }
    for(i=0;i<vt[tnode].size();i++) { // 새로 추가된 노드에서 뻗어진 간선으로 다음 노드 탐색
